...
tt = datestr(now, 30);
sav_file = fullfile(sav_dir, ['eval_Testsvm_' tt '.mat']);
cur_diary = fullfile(sav_dir, ['eval_Testsvm_' tt '.diary']);
diary(cur_diary)
...
for j = 1 : ntag
    % linear SVM baseline
    curid = (R(:,j)~=0) ;
    [mod_final, ~] = cv_svm_wrapper2(1.*(R(curid,j)==.5), X(:,curid)', ...
        'do_normalize', false, 'kernel_type', 0, 'gam', 0, 'neg_pos_ratio', 10);
...
% 
% fprintf(1, ' test AP with svm %0.4f, with knn %0.4f, prior %0.4f\n', po.ap, pk.ap, po.prior);
...
for j = 1 : ntag
    % evaluate
    [~, ~, y1] = svmpredict(imglab(:,j), Xtest', svm_param{j});
