...
def get_wn_similarity(sim, wn_list, offset_dict):
...
def compute_dispersion(argv):
    ...
    parser.add_option('', '--output_file', dest='output_file', default="tag_dispersion.csv", help='name of the ouptut file')
    ...
    parser.add_option('', '--end_idx', dest='end_idx', type='int', default=-1)
    ...
    sim = shelve.open(shelve_file, writeback=False)
    ...
    for tag in tlist:
        cnt += 1
        if cnt < opts.start_idx: 
            continue
        if opts.end_idx > 0 and cnt > opts.end_idx:
            continue
        ...
        if idx_list:
            wn_sim = get_wn_similarity(sim, wn_list, offset_dict)
            ...
            if PRINT_FLAG:
                tt = datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')
                print "%s %d/%d %s\t sim %0.4f\t #synsets %d\t #pairs %d" % (tt, cnt, nt, tag, avg_sim, numwn, npair)
            fo.write("%s,%d,%0.4f\n" % (tag, numwn, avg_sim) )
        else:
            if PRINT_FLAG:
                tt = datetime.strftime(datetime.now(), '%Y-%m-%d %H:%M:%S')
