...
Klist = [5 7]; %[3 5 7] ; %9:-2:5;
alpha_list = 100 ; %[1 10 100 1000 10000]; 

max_iter = 10; % debug use
...
tt = datestr(now, 30);
sav_file = fullfile(sav_dir, ['eval_multilabel_' tt '.mat']);
cur_diary = fullfile(sav_dir, ['eval_multilabel_' tt '.diary']);
diary(cur_diary)
...
for K = Klist
    for alph = alpha_list
        ...
        %% learn with uniform features
        [Uu, Vu] = matchbox(R, X, Yu, alph, 'indR', id_train, 'k', K, 'max_iter', max_iter);
        Ru = X'*Uu'*Vu*Yu;
        pu = compute_perf(Ru(id_test), 1.*full(cur_lab(id_test)), 'store_raw_pr', 2);
        ...
        fprintf(1, ' test AP with matchbox %0.4f, with I-feature %0.4f, with knn %0.4f, prior %0.4f\n', ...
                po.ap, pu.ap, pk.ap, po.prior);
        ...
        % unif feature
        Rnu = Xnew'*Uu'*Vu*Yu;
        pnu = compute_perf(Rnu(:), 1.*full(labnew(:)), 'store_raw_pr', 2);
        ...
        fprintf(1, ' AP on new data: with matchbox %0.4f, with I-feature %0.4f, with knn %0.4f, prior %0.4f\n', ...
            pn.ap, pnu.ap, pkn.ap, pn.prior);
        ...
        % unif feature
        Ru5 = X5'*Uu'*Vu*Yu;
        pu5 = compute_perf(Ru5(:), 1.*full(lab5(:)), 'store_raw_pr', 2);
        
        fprintf(1, ' AP on new data w. 5+ tags: matchbox %0.4f, with I-feature %0.4f, knn %0.4f, prior %0.4f\n', ...
            p5.ap, pu5.ap, pk5.ap, p5.prior);
        % evaluate top K precision
        R5_k = R5_k + 0.001*rand(size(R5_k)); % randomly break ties
        [tp5, tk5, tu5] = deal(ones(n,1));
        topk = 5;
        for i = 1 : n
        ...
        fprintf(1, '  matchbox: %0.4f, [%s]\n', mean(tp5), num2str(hist(tp5, 0:.2:1)));
        fprintf(1, '  I-feature: %0.4f, [%s]\n', mean(tu5), num2str(hist(tu5, 0:.2:1)));
