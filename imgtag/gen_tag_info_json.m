...
json_out = fullfile(data_dir, 'wordnet_tag.json');
db_out = fullfile(data_dir, 'taginfo.db');
...
insert_tmpl = 'INSERT INTO tag_score (tag,count,infm,cnt_prc,infm_prc) VALUES (''%s'',%d,%f,%f,%f)';
dbid = mksqlite('open', db_out);
mksqlite(dbid, 'DELETE FROM tag_score');
for i = 1 : nv
    stmt = sprintf(insert_tmpl, vocab{i}, vcnt(i), vscore(i), vfprct(i), vsprct(i));
    
    mksqlite(dbid, stmt);
    if mod(i, 1000)==0
        fprintf(1, '%s entry %d/%d: %s\n', datestr(now,31), i, nv, stmt);
    end
end
res = mksqlite('SELECT count(*) FROM tag_score');
disp(res);
mksqlite('close');


%% json output
vstruct = struct('word', vocab, 'x', mat2cell([vfprct(:), vsprct(:)], ones(1,nv), [2]), ...
    'cnt', num2cell(vcnt), 'ifm', num2cell(vscore(:)));
fo = fopen(json_out, 'wt');
fprintf(fo, '[\n');
for i = 1 : nv-1
    fprintf(fo, '%s ,\n', mat2json(vstruct(i)) );
end
fprintf(fo, '%s \n]\n', mat2json(vstruct(end)));
fclose(fo);
