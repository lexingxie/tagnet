%% EXP 1.a -- try significantly more training data 
% for tag recommendataion, or multi-tag labeling
% run the best config to date

NUMV = 150;
NUMI = [1000   2000   3000  1000]; %[200 200 3000 3000];
...
tt = datestr(now, 30);
sav_file = fullfile(sav_dir, ['eval_MultiTraining_' tt '.mat']);
cur_diary = fullfile(sav_dir, ['eval_MultiTraining_' tt '.diary']);
diary(cur_diary)
...
for K = Klist
    for alph = alpha_list
        ...
        fprintf(1, ' AP on new data: with matchbox %0.4f, with knn %0.4f, prior %0.4f\n', pn.ap, pkn.ap, pn.prior);
