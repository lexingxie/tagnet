...
function [U, V, predR] = matchbox(R, X, Y, alpha, varargin)

%% optimize R ~ X'*U'*V*Y

[k, indR, initU, initV, szR, fmin_iter, max_iter, gradobj, solver] = process_options(varargin, ...
    'k', 5, 'indR', [], 'initU', [], 'initV', [], 'szR', [], ...
    'fmin_iter', 2, 'max_iter', 15, 'gradobj', 'on', 'solver', 'fminunc');
...
prev_val = compute_err(vec_u, vec_v, Rr, Xr, Yr, indRr); 
delta = prev_val + 1;
fprintf('%s initial err=.5|R-X''U''VY|^2=%f, on %d values\n', datestr(now, 31), prev_val, num_vr);
...
u_x = zeros(k, n); % k x n -- now every X(:, i) is used 
v_y = zeros(k, m); % k x m
parfor i = 1 : n
    u_x(:, i) = U*X(:, i);
end
...
for i = 1 : num_vr
    ...
    cure = Rr(i) - u_x(:, indR(i, 1))'*v_y(:, indR(i,2)) ; % cached version
    total_err = total_err + cure^2;
    
    % compute gradient    
    if target=='U'
        %grado = grado - cure*V*Y(:, indR(i,2))*X(:, indR(i,1))' ;
        grado = grado - cure*v_y(:, indR(i,2))*X(:, indR(i,1))' ;
        %grado = grado - cure*V*yy(:, i)*xx(:, i)' ;
    else % V
        grado = grado - cure*u_x(:, indR(i,1))*Y(:, indR(i,2))' ;
...
obj = .5*(total_err + alpha*(vec_u'*vec_u + vec_v'*vec_v));
